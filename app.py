...
def init_db():
    ...
    cur.execute("""
        ...
        CREATE TABLE IF NOT EXISTS auditoria (
            ...
            accion TEXT, observacion TEXT DEFAULT '',
            fecha TEXT DEFAULT (NOW()::text)
        );
        CREATE TABLE IF NOT EXISTS historial (
            id SERIAL PRIMARY KEY, tarea_key TEXT, usuario_id INTEGER,
            tipo TEXT, detalle TEXT DEFAULT '',
            fecha TEXT DEFAULT (NOW()::text)
        );
    """)
    ...
    if not cur.fetchone():
        cur.executemany(
            ...
            [
                ...
                ("director","Direccion ESUCAR",generate_password_hash("director2025"),"direccion","[]"),
...
def subir_evidencia():
    tk = request.form.get("tarea_key", "")
    desc = request.form.get("descripcion", "").strip()
    eje = request.form.get("eje_id", "")
    obj = request.form.get("obj_id", "")
    year = int(request.form.get("year", 2025))
    mes = int(request.form.get("mes_idx", 0))
    tidx = int(request.form.get("tarea_idx", 0))
    u = current_user()
    if u["rol"] == "responsable":
        ei = json.loads(u["eje_ids"])
        if ei and eje not in ei:
            return jsonify({"error": "Sin permisos"}), 403
    if not desc:
    ...
    if "archivo" in request.files:
        ...
        if f and f.filename and allowed_file(f.filename):
            ao = secure_filename(f.filename)
            au = str(uuid.uuid4())
            am = f.content_type or ""
            ad = base64.b64encode(f.read()).decode()
    ...
    if cur.fetchone():
        cur.execute("UPDATE tareas_estado SET estado='enviada',actualizado=%s WHERE tarea_key=%s", (now, tk))
    else:
        cur.execute("INSERT INTO tareas_estado (tarea_key,eje_id,obj_id,year,mes_idx,tarea_idx,estado,actualizado) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)", (tk, eje, obj, year, mes, tidx, "enviada", now))
...
def descargar_archivo(archivo_uuid):
    ...
    cur.close()
    conn.close()
    if not ev or not ev["archivo_data"]:
        return jsonify({"error": "No encontrado"}), 404
    return Response(base64.b64decode(ev["archivo_data"]), mimetype=ev["archivo_mime"] or "application/octet-stream", headers={"Content-Disposition": f'attachment; filename="{ev["archivo_orig"]}"'})
@app.route("/api/auditoria", methods=["POST"])
@rol_required("auditor")
def registrar_auditoria():
    data = request.get_json()
    tk = data.get("tarea_key", "")
    accion = data.get("accion", "")
    obs = data.get("observacion", "").strip()
    u = current_user()
    if accion not in ("validada", "rechazada"):
        return jsonify({"error": "Accion invalida"}), 400
    if accion == "rechazada" and not obs:
        return jsonify({"error": "Indique motivo del rechazo"}), 400
    now = datetime.now().isoformat(sep=" ", timespec="seconds")
    conn = get_db()
    cur = conn.cursor()
...
def cambiar_password(uid):
    ...
    if len(nueva) < 6:
        return jsonify({"error": "Minimo 6 caracteres"}), 400
    conn = get_db()
    cur = conn.cursor()
